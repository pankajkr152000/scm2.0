 ...
 */
@Service
public class UserSignupFormServiceImpl implements IUserSignupFormService {
    ...
    public UserSignupFormServiceImpl(IUserRepository userRepositories, PasswordEncoder passwordEncoder, UserIdGeneratorService userIdGeneratorService) {
    ...
    public User saveUser(User user) {
        ...
        if(isUserExistsByEmail(userEmail)) {
            log.error("Email already exists : {}", userEmail);
            throw new AppRuntimeException(ErrorCodes.USER_EMAIL_ALREADY_EXISTS);
        }
        
        if(isUserExistsByContactNumber(user.getContactNumber())) {
            log.error("Contact Number already exists : {}", user.getContactNumber());
            throw new AppRuntimeException(ErrorCodes.USER_PHONE_NUMBER_ALREADY_EXISTS);
        }

        log.info("Saving new user: {}", userEmail);
    ...
    public Optional<User> updateUser(User newUser) {
        ...
        User existingUser = userRepository.findById(newUser.getUserId())
                .orElseThrow(() -> new AppRuntimeException(ErrorCodes.RESOURCE_NOT_FOUND));

        // update fields
        existingUser.setFirstName(newUser.getFirstName());
        existingUser.setLastName(newUser.getLastName());
        existingUser.setEmail(newUser.getEmail().toLowerCase());
    ...
    public boolean isUserExistsByEmail(String email) {
       ...
       //////////                                           /////////////////////////
        ...
        boolean isUserExistBySameEmail = userRepository.findByEmail(userEmail).isPresent();
